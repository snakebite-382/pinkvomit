...
export function renderLikeButton(liked: boolean, postID: ID) {
  ...
  <svg 
    ...
    hx-vals='{ "post": "${postID}" }' 
    id="post-like-button-for-${postID}"
...
export function renderCommentButton(postID: ID) {
  ...
  <svg 
    ...
    id="post-comment-button-for-${postID}">
...
export function renderComment(comment: TimelineComment) {
  ...
  <div class="comment" id="comment-${id}">
    <div class="commenter" id="comment-commenter-for-${id}"><a id="comment-blog-title-for-${id}" href="/blogs/view/${blogTitle}">${blogTitle}</a></div>
    <div class="comment-content" id="comment-content-for-${id}">${content}</div>
    <button class="reply-button" id="comment-reply-button-for-${id}">reply</button>
    <div class="replies" 
      id="comment-replies-for-${id}"
      hx-trigger="load" 
      hx-target="this"
      hx-swap="beforeend"
...
export function renderReply(reply: TimelineReply) {
  ...
  <div class="reply" id="reply-${id}">
    <div class="replier" id="reply-replier-for-${id}">
      <a id="reply-blog-title-for-${id}" href="/blogs/view/${blogTitle}">${blogTitle}</a> : <a href="/blogs/view/${atBlog}">@${atBlog}</a>
    </div>
    <div class="reply-content" id="reply-content-for-${id}">${content}</div>
    <button class="double-reply-button" id="reply-double-reply-button-for-${id}">reply</button>
  </div>`
}
...
export function renderPost(post: TimelinePost, minimal: boolean = false) {
  ...
  <div id="post-${id}" class="post" timestamp="${Date.parse(created_at.toISOString())}">
    <div class="post-header" id="post-header-for-${id}">
      <a id="post-blog-title-for-${id}" href="/blogs/view/${encodeURIComponent(blogTitle)}">${blogTitle}:</a>
    </div>
    <div class="markdown" id="post-markdown-for-${id}">
      ${renderMarkdown(content)}
    </div>
    ${minimal ? "" : `
    <div class="interactions" id="post-interactions-for-${id}">
      <div class="like-interaction" id="post-like-interaction-for-${id}">
        ${likeCount}
        ${renderLikeButton(likedByBlog, id)}
      </div>
      <div class="comment-interaction" id="post-comment-interaction-for-${id}">
        ${commentCount}
    ...
    <div 
      ...
      id="post-comment-section-for-${id}"
    >
      <div class="comments"
        hx-trigger="load"
        ...
        hx-swap="beforeend"
        hx-target="this"
        id="post-comments-for-${id}"
      ></div>
      <div class="comment-input" id="post-comment-input-for-${id}">
        <form hx-post="/posts/api/create/comment" hx-target="previous .comments" hx-swap="beforeend" id="comment-form-for-${id}">
          ...
          <div id="post-commenter-reply-name-for-${id}" class="commenter-reply-name"></div>
          <button id="post-stop-replying-for-${id}" class="hidden stop-replying-button" >stop replying</button>
        </form>
      </div>
    </div>
    `}
